...
const TestimonialsSection = () => {
  ...
  const { data: testimonials = [] } = useSWR("testimonials", () =>
    testimonialsAPI.getAll().then((r) => r.data.testimonials || [])
  );
  ...
  return (
    <div className="py-24 bg-white">
      <div className="max-w-7xl mx-auto px-6">
        <RevealOnScroll>
          <div className="text-center mb-16" style={{ maxWidth: "600px", margin: "0 auto 4rem auto" }}>
            <div
              style={{ color: primaryColor }}
              className={`text-sm font-bold uppercase tracking-wider mb-2 ${!resolvedHex ? getThemeClass("text", "600") : ""}`}
            >
              Testimonials
            </div>
            <h2 className="text-4xl font-bold text-gray-900 mb-4">Trusted By Thousands</h2>
            <p className="text-gray-600">See what our members and partners say about the impact of Arova in their daily lives.</p>
          </div>
        </RevealOnScroll>
          ...
          {testimonials.map((item, idx) => (
            <RevealOnScroll key={item.id} delay={idx * 150}>
              <div className="bg-gray-50 p-8 rounded-2xl shadow-lg border border-gray-100 relative h-full flex flex-col">
                ...
                <p className="text-gray-600 mb-6 italic relative z-10 flex-grow">"{item.text}"</p>
